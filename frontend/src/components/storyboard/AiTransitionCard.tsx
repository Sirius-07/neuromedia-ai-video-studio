...
export const AiTransitionCard: React.FC<AiTransitionCardProps> = ({
  ...
  const getProxiedVideoUrl = (originalUrl: string | undefined) => {
    ...
    return `http://localhost:4300/api/v1/proxy/video?url=${encodeURIComponent(originalUrl)}`;
  };
  ...
  const handleGeneratePrompt = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:4300/api/v1/transition/generate-prompt', {
        method: 'POST',
        headers: {
